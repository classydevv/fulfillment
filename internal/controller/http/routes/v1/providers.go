...
func (r *providerRoutes) providerCreate(ctx *fiber.Ctx) error {
	...
	providerId, err := r.uc.Save(ctx.UserContext(), entity.Provider{
		Id: request.Id,
		Name: request.Name,
	})
	if err != nil {
...
type providerGetAllResponse struct {
	Providers []entity.Provider `json:"providers"`
}

// @Summary     Show all providers
// @Description Get all available providers registered in the system
// @ID          providerGetAll
// @Tags  	    provider
// @Accept      json
// @Produce     json
// @Success     200 {object} providerGetAllResponse
// @Failure     500 {object} responseError
...
func (r *providerRoutes) providerGetAll(ctx *fiber.Ctx) error {
	...
	return ctx.Status(http.StatusOK).JSON(providerGetAllResponse{providers})
}